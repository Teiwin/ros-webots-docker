...
# select only the sensor we want from environment variables
print("Controller initialized : publishing in ROS topics :")
none = True
LIDAR = getenv("LIDAR", 1)
CAMERA = getenv("CAMERA", 1)
TOF = getenv("TOF", 1)
if LIDAR == "1":
    rospy.Timer(rospy.Duration(TIME_SCAN), rc.publish_scan)
    print("-/LidarScan : Lidar datas")
    none = False
if CAMERA == "1":
    rospy.Timer(rospy.Duration(TIME_IMG),  rc.publish_image)
    print("-/ImageScan : Camera datas")
    none = False
if TOF == "1":
    rospy.Timer(rospy.Duration(TIME_SENS), rc.publish_sensors)
    print("-/SensorsScan : TOFs datas")
    none = False

if none:
    print("Not publishing in any topic")
